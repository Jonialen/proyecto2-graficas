...
/// Triángulo optimizado con precálculos
pub struct Triangle {
    pub v0: Vector3,
    pub v1: Vector3,
    pub v2: Vector3,
...
impl Triangle {
    ...
    fn barycentric(&self, point: &Vector3) -> (f32, f32, f32) {
        ...
        let d20 = v0p.dot(self.edge1);
        let d21 = v0p.dot(self.edge2);
...
impl RayIntersect for Triangle {
    ...
    fn ray_intersect(&self, ray_origin: &Vector3, ray_direction: &Vector3) -> Intersect {
        // Early exit con AABB
        if !self.bounds.intersect(ray_origin, ray_direction) {
            return Intersect::empty();
        }

        // Möller-Trumbore optimizado
        let h = ray_direction.cross(self.edge2);
...
/// Malla optimizada con BVH interno
pub struct Mesh {
